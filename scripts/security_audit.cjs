...
const IGNORE_DIRS = new Set(['.git', 'node_modules', 'dist', 'models', 'build-llama', '.0xagent', 'llama', 'build', 'logs', '__pycache__', '.pytest_cache', 'tmp', 'temp', 'data']);
const IGNORE_EXTS = new Set(['.png', '.jpg', '.jpeg', '.ico', '.gguf', '.exe', '.dll', '.so', '.dylib', '.bin', '.safetensors', '.mmproj', '.onnx', '.tsbuildinfo', '.zip', '.tar', '.gz', '.pyc', '.pyo']);
