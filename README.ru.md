...
# CLI-протокол персонального AI-ассис### 🤖 Модуль «Вероника» — Персональный AI-ассистент и Telegram-супервизор
...
- **Интеграция с Web-IDE (Панель действий и Модальное окно)**: Постановка автономных задач с паспортом проекта, уровнем автономии (L0–L5) и кастомными промптами прямо из веб-интерфейса.
- **Сверхплотный Context Engine и 4-фазные промпты**: Синтезирует компактные выжимки состояния проектов (~150-250 токенов) через команду `0xagent veronica context <project>` и внедряет паспорт проекта в промпты агентов.
...
### 🧠 Двойной инференс и единый селектор моделей
- **Унифицированный мультимодельный селектор**: Мгновенное переключение между облачными моделями Antigravity (Gemini 3.7/3.6/3.1 Pro, Claude Sonnet 4.6 и Opus Thinking, GPT-OSS 120B) и локальными GGUF с инлайн-настройкой рассуждений (`off`, `low`, `medium`, `high`).
- **24-часовой кэш моделей**: Высокопроизводительное кэширование в `localStorage` с авто-инвалидацией и принудительным обновлением в 1 клик.
...
- **Приватный веб-поиск и скоростной файловый индекс**: Встроенный локальный поиск SearXNG / DuckDuckGo со скрейпером Markdown и нативный быстрый поиск файлов FFF на Rust (`@ff-labs/fff-node`, <3 мс).� уровня L3+.
